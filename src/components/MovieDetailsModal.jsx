...
import { X, Star, Calendar, Clock, Globe, Play, Users, DollarSign, ChevronDown, ChevronUp } from "lucide-react"
...
export function MovieDetailsModal() {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      ...
      try {
        const [detailedMovie, movieImages, watchProviders] = await Promise.all([
  ...
  if (!movie || isLoadingDetails) {
    // Optionally render a loading state here if needed, otherwise return null
    return (
  ...
  const allLanguages = movie.spoken_languages || []
    ...
    >
      <div className="bg-gray-900 border border-gray-700 rounded-3xl max-w-6xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-8">
          ...
          {movie.watchProviders && <WhereToWatch watchProviders={movie.watchProviders} movieTitle={movie.title} />}

          {/* Movie Images Gallery */}
          {movie.images && (movie.images.backdrops?.length > 0 || movie.images.posters?.length > 0) && (
            <MovieImageGallery
              images={[...(movie.images.backdrops || []), ...(movie.images.posters || [])]}
